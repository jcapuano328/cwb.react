...
var Stragglers = require('./stragglers');
...
let findResult = (firepoints, attackupslope, attacknight, attacklowammo, attackdg, attackccflank, attackswamp,
						defendtrench, defendcolumn, defendmounted, defendmoralestate) => {
	if (attackdg) {
		firepoints /= 2;
	}

	var index = results.findIndex((r) => firepoints >= r.low && firepoints <= r.high);
